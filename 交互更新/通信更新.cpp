...
int minCostId = myid;			//存储有最小答案的id号
...
// 更新最优解
void CheckOptimalAnswer(bool tag = 0) {

	MPI_Status status;
	int flag;
	int recvCost;
	MPI_Iprobe(MPI_ANY_SOURCE, NEW_COST, MPI_COMM_WORLD, &flag, &status);
	...
	while (flag) {
		MPI_Recv(&recvCost, 1, MPI_INT, status.MPI_SOURCE, NEW_COST, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
		if (recvCost < minCost || (recvCost == minCost && status.MPI_SOURCE > minCostId)) {

			minCost = recvCost;
			
			minCostId = status.MPI_SOURCE;
		}
		MPI_Iprobe(MPI_ANY_SOURCE, NEW_COST, MPI_COMM_WORLD, &flag, &status);
	}
}
...
//发送最优解
void SendOpitmalAnswer() {
	for (int i = 0; i < numprocs; ++i)
	{
		if (i != myid) {
			MPI_Send(&minCost, 1, MPI_INT, i, NEW_COST, MPI_COMM_WORLD);
		}
	}
...
/*
	...
	若当前距离 > minCost 时弹出
*/
void SearchWhileSending(list<int>& lst, int& src, int& cost, int& nodeCnt) {
...
int main(int argc, char* argv[]) {
	...
	SearchWhileSending(tourList, src, total_cost, nodeCnt);	//	开始搜索
